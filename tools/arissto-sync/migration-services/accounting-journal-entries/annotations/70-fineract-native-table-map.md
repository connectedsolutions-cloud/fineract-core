...
| `entry_date` | `CNT_PERIODO.ANIO/MES` plus `CAST(CNT_PARTIDAS.FECHA_PARTIDA AS date)` | Preserve the source date when it lies inside the accounting period; otherwise use the period's final calendar day. It must be before the cutoff and after any applicable target closure unless the dedicated import policy explicitly handles historical closures. The unmodified source date remains in header provenance. |
