...
service is executable for controlled local full-scope work, but production
promotion is disabled. Passing unit tests, successful canaries, and the
assembled clean-cycle target prove the implementation; they do not replace the
final single-run reconciliation and unchanged replay required to close Gate 5.
...
1. Build one fresh full-scope plan after the fixes proved during cycle
   `sandbox-2026-09-05-clean-a`, apply or recover every supported loan, and run
   strict full-population reconciliation. The successful state assembled by a
   full run, a full replay, and a scoped repair is strong implementation
   evidence, but it is not one zero-failure reconciled run.
2. Review the 19 planned quarantines as the complete explicit no-write set: 18
   fully reversed refinance attempts and superseded source-error shell `2120`.
   Any additional quarantine must be independently explained before promotion.
3. Replay the accepted full plan unchanged and prove zero new loans,
   transactions, charges, paid-by rows, transfers, schedules, or journal
   entries.

### Fresh full-cycle evidence — 2026-09-05 through 2026-09-08
...
The target therefore contains all 2,493 supported loans from that frozen
population, with 19 reviewed no-write quarantines. Gate 5 remains open because
the final repaired state has not yet been exercised and reconciled as one
fresh full run followed by one unchanged full replay.
...
| `G5-SCH-009` | `PARTIAL` (acceptance blocked) | Residual operator-adjusted schedules with archived versions | Nine loans: closed/state-`3` `23`, `90`, `317`, `340`, `359`; active/state-`1` `479`, `1738`, `1841`, `1869` | Every loan has `CRD_REESTRUCTURACION` history and one or more archived plans. The five closed loans use planner policy `reviewed-manual-adjustment`, whose behavior is historical-reference-only: Fineract retains its native schedule and any difference remains a visible reviewed variance. The four active loans use `exact-source-schedule` with guarded writer `fineract-source-exact-active-schedule-v1`. Fresh combined-cohort plan `cfd5f875dbae4ef4bf30aa7da04ea055` expanded the nine loans to 19 dependency-complete loans. Run `7e471a3392ff47b6a6898e4d03352a49` succeeded for all 19 and strict reconciliation returned `ok=true`; unchanged replay `f7b7235a42684e96bfb5227a9348873a` recovered all 19 and reconciled identically, proving replay idempotency. The active schedules are exact and the closed schedule differences retain the reviewed classification. Archived operations are not replayed and no calculator rule is inferred. [Source evidence](../../../../../../credesal-db-space/docs/learnings/prestamos-inconsistencia-base-360-365.md#limite-para-descartar-ajustes-manuales). | The final checker returned `accepted=false` for both reconciled runs. Evidence, membership, policies, completion, quarantine, schedule import, blocking findings, and replay are clean, but 20 non-schedule balance variances remain: active loans retain interest/fee/penalty, total-outstanding, or cutover-total differences after source-exact schedule import and servicing replay; closed loans `23`, `317`, and `359` retain fee-balance differences. Close only after those differences are either fixed and reconciled or the contract is revised with source evidence proving that a reported field is not cutover-comparable. The checker must then return `accepted=true`; no separate closed-subgroup apply is required. |
...
| `G5-SCH-014` | `PARTIAL` | Residual source-exact schedule-preview mismatches from the latest composed sync | Contractual-origin candidates from the original direct failures, plus the separate line-`00001` loan `3` signature | The source-derived contractual-anchor classifier and guarded post-disbursement, pre-servicing `sourceExactActiveSchedule` path are implemented. Full replay plan `9995d2e755894af897cfbe133888d0c3` recovered 30 active-writer contractual-anchor loans and 20 reviewed closed refinance roots; the only contractual-anchor quarantine was reviewed voided successor `638`. Loan `3` was independently reconciled and replayed cleanly in runs `1d9c9fbd63a746edbded07736bbf3cee` and `bb21edbc3df245c9a1238f6d3f5240ea`. | No known schedule-preview implementation failure remains. Close together with `G5-OPS-002` after strict full-population reconciliation confirms the final repaired target, then retain the cohort in the unchanged full replay. |
| `G5-LCY-001` | `PARTIAL` | Refinance predecessors left open when their successor did not complete | Historical population of 29 predecessor symptoms | The symptom was dependency propagation, not a standalone predecessor defect: the successor atomically creates the final `4/00019 PAGO CREDITO-REFINANCIAMIENTO`. The clean-cycle full run and replay completed the supported refinance population; the last unrelated recovery pair `1507 -> 1663` was repaired and reconciled with zero mismatches in run `7968a558d28747949bded7500bb1eaee`. [Source interpretation](../../../../../../credesal-db-space/docs/learnings/refinanciamientos.md#10-interpretacion-de-la-cohorte-historica-g5-lcy-001). | Do not add a predecessor repair. Close with `G5-OPS-002` when one fresh full reconciliation proves every supported completed successor created the exact payoff, closed its predecessor, and produced balanced journals. |
...
| `G5-OPS-001` | `PARTIAL` | Clean strict lifecycle acceptance matrix | All supported lifecycle classes on fresh identities in cycle `sandbox-2026-09-05-clean-a` | The full clean-cycle plan exercised 2,493 supported loans across active, paid, approved-undisbursed, ordinary and Cobro Movil repayment, reversal, insurance, terminal, refinance, consolidation, and partial-paydown paths. All supported identities now exist after the full run, full replay, and scoped repair; only the 19 reviewed no-write quarantines remain. | One fresh full run must finish with zero supported failures and receive strict full-population reconciliation. This is the remaining matrix acceptance proof, not missing lifecycle implementation. |
...
`1507 -> 1663` failure. The remaining Gate 5 work is the single fresh
full-population reconciliation and unchanged replay described in the current
status section, not another schedule-writer implementation pass.
...
schedule/API edge population successfully. The current remaining work is the
single fresh zero-failure full reconciliation and unchanged replay listed in
the current-status section above. Complete Mobile Collections repayment
linking remains downstream of that accepted loan run.
