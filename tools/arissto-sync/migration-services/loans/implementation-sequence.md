...
The loans service has completed Gates 1 through 5. Gate 5 was reopened when the
contract was tightened to require exact source installment dates and component
amounts plus exact historical repayment allocations, and was accepted again
after the fresh/clean workflow evidence recorded below. The sequence preserves
the implementation and promotion evidence that must remain intact.

## Current status — 2026-09-13

**Overall status: available.** Fresh/clean cycle
`sandbox-2026-09-13-loans-b` completed the dependency-closed
`clients -> employees -> loans` workflow on the disposable `sandbox` tenant.
The frozen loans plan has no remaining failed or dependency-blocked item, its
19 quarantines exactly match the reviewed no-write set, and strict recovery
reconciliation returned `ok=true` with zero blocking mismatches. Registry
availability does not waive target-specific preflight, immutable plan review,
dependency gating, scheduler pause, apply journaling, or reconciliation.
...
- source-derived omission of fully reversed, net-zero refinance attempts with
  preserved provenance and fail-closed quarantine for partial signatures;
- atomic handling of single- and multi-predecessor refinancing, including the
  reviewed cross-client and partial-paydown shapes, plus loan `83`'s proved
  component reallocation; and
- deterministic external identities, bounded retry selection, accounting
  checks, and downstream repayment identity ownership; and
- bounded concurrency across independent loans, with sequential commands
  inside each lifecycle, refinance dependency ordering, worker-local HTTP
  sessions, main-thread state journaling, and a shared Fineract recovery pause.

Gate 5 acceptance evidence:

1. Workflow plan `2b71c16f0d6a411bbfd2b1d5e526cd62` and workflow run
   `dfcff1a3246e46828a90972b6630fbde` belong to fresh/clean cycle
   `sandbox-2026-09-13-loans-b`, target fingerprint `b9f6834b66790744`, and
   frozen loans plan `c47e6d2d3e644ba6929223b03c2e5789`.
2. The host disk filled during the initial apply. After capacity was restored,
   the same workflow, cycle, and frozen plan resumed without resetting or
   replacing the target. Recovery child run
   `a34a602cb5ec413eaf6c6023e0d56b74` recovered 109 existing actions and safely
   replayed 53 dependency prerequisites with no failed or blocked item.
3. The durable plan journal closes with 2,509 succeeded loans, 19 recovered
   loans, 19 reviewed no-write quarantines, and two succeeded products. Strict
   recovery reconciliation covered 162 loans and returned zero blocking
   mismatches, while the completed workflow re-verified the accounting boundary.
4. Promotion accepts same-plan recovery plus the existing unchanged canary
   matrix as the Gate 5 idempotency proof. A future portfolio-wide zero-write
   replay remains valuable regression hardening, but is not a registry-status
   blocker. This decision is explicit because the environmental failure made a
   synthetic uninterrupted-run requirement less informative than successful
   durable recovery of the exact frozen plan.

### Accepted fresh full-cycle evidence — 2026-09-13

The accepted cycle supersedes the earlier assembled target described below.
It is the first fresh/clean dependency-complete workflow accepted for Loans.
All earlier failures remain in the immutable workflow history for audit, while
the current durable plan journal contains no unresolved supported item.

### Superseded clean-cycle evidence — 2026-09-05 through 2026-09-08

The active cycle ledger at
`.arissto-sync/cycles/sandbox-2026-09-05-clean-a/state.sqlite3` supersedes the
older historical full-run counts below:

- full plan `0d7a06d33d8e4504824092f62aaba75d` contained 2,514 actions;
  run `7735934d63e54eca9a5dffd920eaae33` succeeded for 2,492 loans and two
  products, emitted the 19 reviewed quarantines, and failed only loan `2374`;
- full replay plan `9995d2e755894af897cfbe133888d0c3` recovered loan `2374`
  and 2,491 other loans, kept the same 19 reviewed quarantines, left both
  products unchanged, and failed only loan `1663` on cutover-insurance recovery;
- scoped recovery run `7968a558d28747949bded7500bb1eaee` recovered predecessor
  `1507` and loan `1663`; strict reconciliation returned `ok=true`, two loans,
  one product, two staff assignments, zero failed/quarantined items, and zero
  mismatches.

That target contained all 2,493 supported loans from its frozen population,
with 19 reviewed no-write quarantines. It remains useful historical evidence,
but no longer controls the registry status after the accepted 2026-09-13 cycle.
...
multi-predecessor successors, including the reviewed cross-client and partial-
paydown shapes, are implemented and canary-proved through their dedicated
...
## Gate 5: exact reconciliation and promotion — complete
...
- `PARTIAL`: implementation exists and has passed material execution evidence,
  but one explicitly named acceptance proof is still missing;
...
| `G5-SCH-003` | `COMPLETE` | Reviewed operator-adjusted non-monotonic schedules and one source duplicate-date shape | Live audited population of 9 loans. Reviewed manual exceptions: `945`, `1117`, `1743`, `1748`, `2069`, `2241`, `2254`, and `2355`; separately classified source-exact loan: `2374`. | The eight reviewed exceptions have operator-created `CRD_REESTRUCTURACION` rows and archived plans, so Fineract retains its native schedule while preserving those schedules as reviewed history. Loan `2374` has no adjustment history: its final two source rows share one due date and the last is principal-only. The guarded active-schedule path aggregates only that exact terminal pair into one period and rejects near-matches. [Source evidence](../../../../../../credesal-db-space/docs/learnings/prestamos-inconsistencia-base-360-365.md#planes-actuales-con-fechas-de-cuota-no-monotonicas). | Satisfied by focused tests and the clean-cycle evidence. Loan `2374` was the sole failed supported loan in run `7735934d63e54eca9a5dffd920eaae33`; after the cardinality fix, full replay `b946bacae2d54fbfa4bdf3610eb77d44` recovered it successfully. It is no longer an open quarantine or unclassified schedule defect. |
...
| `G5-SCH-008` | `COMPLETE` | Contractual first-period accrual anchor differs from the financial disbursement event or nominal repayment interval | Derived at plan time from actions where `contractual_schedule_anchor_policy.classification == source-exact-contractual-origin-anchor`; never freeze a loan-ID list or the historical count 48 | The smallest native representation preserves the real disbursement date, anchors contractual interest at `CRD_CARTERA.FECHA_OTORGAMIENTO`, composes with the inclusive-first-day rule, and uses the guarded active source-exact writer when native inference differs. Fresh namespaced plan `817bb2ce525945b59e9358a89551e351` derived 51 current classifier members and expanded them to a dependency-complete 176-loan graph: 50 supported members plus reviewed voided successor `638`. Post-fix run `2ab37f99b9a546b0aa30e76b752b7c45` recovered all 175 supported loans with zero failures; strict reconciliation returned `ok=true` and zero blocking mismatches. Unchanged replay `4e81b505c27443c0badfbc45d7bf69c4` again recovered all 175, created no new loan identities, retained only `638`, and reconciled identically. The executable checker returned `accepted=true` with no incomplete members or schedule findings. [Source evidence](../../../../../../credesal-db-space/docs/learnings/prestamos-inconsistencia-base-360-365.md#cohorte-residual-de-62-previsualizaciones-no-exactas). | Satisfied by the classifier-derived fresh proof, strict reconciliation, and unchanged replay. Retain the cohort checker and the serviced cutover-insurance recovery regression so downstream lifecycle idempotency cannot obscure schedule acceptance again. |
| `G5-SCH-009` | `COMPLETE` | Residual operator-adjusted schedules with archived versions | Nine loans: closed/state-`3` `23`, `90`, `317`, `340`, `359`; active/state-`1` `479`, `1738`, `1841`, `1869` | The five closed loans retain reviewed historical-reference-only schedule differences; the four active loans use the guarded source-exact schedule writer. Combined-cohort run `7e471a3392ff47b6a6898e4d03352a49` and unchanged replay `f7b7235a42684e96bfb5227a9348873a` completed all 19 dependency-closed actions and reconciled with `ok=true`. The previously reported 20 non-schedule component and total variances remain visible under the accepted native-schedule variance policy; they are not blocking mismatches and do not represent missing identities or unbalanced journals. [Source evidence](../../../../../../credesal-db-space/docs/learnings/prestamos-inconsistencia-base-360-365.md#limite-para-descartar-ajustes-manuales). | Accepted by the 2026-09-13 promotion review. Retain the cohort checker and visible variance classifications as regression evidence; no separate closed-subgroup apply is required. |
| `G5-SCH-010` | `COMPLETE` | Terminal source row contains only `MONTO_OTROS` after principal is fully settled | `381`, `1775`, `1871`, `1926`, `2006` | The source has respectively 7, 48, 48, 48, and 48 rows, but each final row carries zero principal and zero interest while retaining only `MONTO_OTROS`. Variable-schedule preview correctly omits that charge-provenance row from the repayment schedule. The engine classifies it as `historical-reference-only` only when there is exactly one terminal zero-core row, its `MONTO_OTROS` is positive, contribution/restructuring/deferred markers are absent, preceding rows allocate the full principal, installment numbers are sequential, and dates are strictly increasing. No installment, payment, adjustment, or charge is synthesized from the row. Near-matches return to the blocking exact-source-schedule path. Loan `1926` is the newly reviewed fifth member; its `2028-04-18` terminal row follows its `2026-06-24` refinance payoff into `2135`, while its actual historical insurance charges remain independently reconstructed. [Source evidence](../../../../../../credesal-db-space/docs/learnings/prestamos-inconsistencia-base-360-365.md#cohorte-residual-de-62-previsualizaciones-no-exactas). | The original four-member cohort was satisfied by regression coverage, nine fail-closed near-match mutations, and sandbox plan `578ee952e59b4515897b585a1c4be190`; run `e74a6f9629644ec0aad3e908f0dde9e3` and replay `39a2022411cb42ae82e0fad855a79fd0` reconciled with zero failures, quarantines, or blocking mismatches. The 2026-09-04 extension is proved by dependency-complete plan `01a804f466784ea2a9bc598ad157056c`, which expanded `3` and `1926` to four loans including the `995 -> 1926 -> 2135` chain. Run `1d9c9fbd63a746edbded07736bbf3cee` and recovery replay `bb21edbc3df245c9a1238f6d3f5240ea` both reconciled with `ok=true`, zero failed/quarantined items, and zero blocking mismatches. |
...
| `G5-SCH-013` | `COMPLETE` | Initial native EMI is below first-period interest | Manual/restructured `301`, `1117`, `1182`; source-exact control `1441` | `301`, `1117`, and `1182` retain stale higher header rates after operator-created lower-rate plans; only these identities use `CRD_CARTERA.INTERES_MONTO_APROBADO` for native creation and keep their edited schedules as provenance. `1441` has no adjustment history: its source first core installment `205.29` covers interest `203.84`, while Fineract's preliminary EMI `202.74` does not. A reviewed `205.31` staging EMI creates the required 75-row cardinality; after its refinancing disbursement components and before servicing, the identity-guarded active-schedule importer installs and strictly verifies the exact source schedule. [Source evidence](../../../../../../credesal-db-space/docs/learnings/prestamos-inconsistencia-base-360-365.md#cuatro-rechazos-de-cuota-menor-que-el-interes). | Satisfied by identity-scoped configuration, fail-closed source-signature and immutable-partial checks, unchanged-path regression coverage, all 309 sync-engine tests, and Fineract provider compilation. Sandbox plan `609f5756c86c4ee49a832abb4fe2fdbb` expanded the four requested loans to eight dependency-complete loans. Recovery run `2f1d7b06f2a34b2388563716d925ba62` completed all eight and reconciled `ok=true` with zero failures, quarantines, or blocking mismatches; only the three reviewed manual cases emit schedule variances, while `1441` is source-exact. Same-plan replay `1a625c8fb63d43bd81f523f35c094b71` recovered all eight without duplicate state and reconciled identically. The canary used tenant `sandbox` backed by `fineract_sandbox`; the local default database was not targeted. |
| `G5-SCH-014` | `COMPLETE` | Residual source-exact schedule-preview mismatches from the latest composed sync | Contractual-origin candidates from the original direct failures, plus the separate line-`00001` loan `3` signature | The source-derived contractual-anchor classifier and guarded post-disbursement, pre-servicing `sourceExactActiveSchedule` path are implemented. Full replay plan `9995d2e755894af897cfbe133888d0c3` recovered 30 active-writer contractual-anchor loans and 20 reviewed closed refinance roots; the only contractual-anchor quarantine was reviewed voided successor `638`. Loan `3` was independently reconciled and replayed cleanly in runs `1d9c9fbd63a746edbded07736bbf3cee` and `bb21edbc3df245c9a1238f6d3f5240ea`. | Closed with `G5-OPS-002`: the accepted fresh/clean workflow left no schedule-preview failure or dependency-blocked supported loan. Retain the cohort in regression coverage. |
| `G5-LCY-001` | `COMPLETE` | Refinance predecessors left open when their successor did not complete | Historical population of 29 predecessor symptoms | The symptom was dependency propagation, not a standalone predecessor defect: the successor atomically creates the final `4/00019 PAGO CREDITO-REFINANCIAMIENTO`. The 2026-09-13 clean-cycle recovery exercised the complete transitive refinance closure, and strict reconciliation correctly accepted successful `FULL_CLOSE` successor effects. [Source interpretation](../../../../../../credesal-db-space/docs/learnings/refinanciamientos.md#10-interpretacion-de-la-cohorte-historica-g5-lcy-001). | Closed with `G5-OPS-002`. Do not add a predecessor repair; retain transitive retry closure and full-close reconciliation as regression cases. |
...
| `G5-GRF-002` | `COMPLETE` | Multi-predecessor loan consolidations | 12 successors, each with exactly 2 active predecessors (24 links) | These are genuine consolidations, not manual adjustments: each pair belongs to the same definitive liquidation, successor principal exceeds the combined source payoffs, and all 12 deliver positive net cash from `$0.34` to `$118.73`. Three successors (`2252`, `2402`, `2406`) consolidate two loans of the successor client; the other nine each include one predecessor owned by another client. Fineract retains the compatibility refinancing header and persists one settlement per predecessor. Ordinary refinancing remains same-client; the approved migration-only cross-client path requires the definitive liquidation, payoff movement, date, and operator and persists both client IDs. The successor's ordinary loan terms independently control rate, payment schedule, and installment count. [Source evidence](../../../../../../credesal-db-space/docs/learnings/refinanciamientos.md#los-12-sucesores-con-dos-anteriores-no-son-ajustes-descartables). [Implementation contract](../../../../docs/LOAN_REFINANCING_AND_CONSOLIDATION.md). | Same-client consolidation is proved by namespaced plan `fe97d37686904e1bbc75e1904ca1e3fa` for `2042 + 2205 -> 2406`, run `3b2e253daee3432dafd03e66a178957e`, and replay `4f8cb9a347a645b98ebc0814d717f891`. Cross-client consolidation is proved by plan `3aa9523256434f1eaf4d974b946b205a` for the seven-loan chain ending in `2472`; run `726183dce4cf4e50ae77b6be219feaf4` and unchanged replay `709cd5eda5bb463caf5e62ab299847ad` both reconciled with `ok=true`, zero failed/quarantined items, and zero mismatches. Replay recovered all seven loans and the product without duplicate financial state. Full plan `0065dc7d540f42c89b5cbe0fe18ec568` contains complete evidence for all 14 direct cross-client settlements and reduces portfolio quarantines from 132 to 57. The API and read-only Mifos view fail closed on incomplete or mismatched evidence. |
| `G5-GRF-003` | `COMPLETE` | Mixed full-close and partial-paydown consolidation | Loan `2402`: predecessor `1784` is fully settled; predecessor `2265` receives `$393.80`, remains active, and has a later `$15.00` payment | The 2026-08-07 definitive liquidation is a real consolidation, but it does not close both predecessors. Treating `2265` as a payoff caused the successor to fail Fineract's transaction-date guard because `2265` correctly has a later 2026-08-12 payment. This is not a manual-adjustment exception and the later payment must remain. [Source evidence](../../../../../../credesal-db-space/docs/learnings/refinanciamientos.md#el-sucesor-2402-combina-una-cancelacion-total-y-un-abono-parcial). | Implemented `FULL_CLOSE`/`PARTIAL_PAYDOWN` settlement persistence, full-close-only native application fields and prepayment preparation, exact atomic transfers for both types, post-write status/balance/allocation checks, read-only Mifos labels, and focused planner/writer/domain tests. Namespaced plan `5fd1b9ac0292496f9a04a6d614bec503` expanded the `1784 + 2265 -> 2402` chain to nine loans with zero blockers or quarantines. Corrected run `0b17c12ffbe64b92bac7e7b3cf1d15ea` and unchanged replay `cdf43c654c484160a235ce3343230379` both reconciled with `ok=true`, zero failed/quarantined items, and zero mismatches. Direct PostgreSQL verification found exactly two settlement rows: closed predecessor `1784` as `FULL_CLOSE` for `$566.65`, and active predecessor `2265` as `PARTIAL_PAYDOWN` for `$393.80`, including its `$0.11` penalty allocation; its later `$15.00` payment remains unreversed. The replay left one copy of each source movement and no duplicate settlement state. |
| `G5-OPS-001` | `COMPLETE` | Clean strict lifecycle acceptance matrix | All supported lifecycle classes in fresh/clean cycle `sandbox-2026-09-13-loans-b` | Frozen plan `c47e6d2d3e644ba6929223b03c2e5789` exercised the dependency-complete supported portfolio. The durable ledger closes with 2,509 succeeded loans, 19 recovered loans, 19 reviewed no-write quarantines, two succeeded products, and no failed or blocked item. | Satisfied by completed workflow `dfcff1a3246e46828a90972b6630fbde` and strict recovery reconciliation with zero blocking mismatches. |
| `G5-OPS-002` | `COMPLETE` | Fresh full supported-population execution and reconciliation | 2,528 synchronized supported loans plus 19 reviewed no-write quarantines | The fresh target was populated under one immutable workflow plan. A host disk-full event interrupted apply, but capacity recovery and corrected transitive retry closure completed the same cycle and plan without a reset. Recovery child `a34a602cb5ec413eaf6c6023e0d56b74` recovered 109 actions and safely replayed 53 prerequisites; the combined durable journal has no unresolved supported loan. | Satisfied by the completed same-cycle workflow, exact reviewed quarantine set, zero failed/blocked durable items, and reconciliation `ok=true` with zero blocking mismatches. |
| `G5-OPS-003` | `COMPLETE` | Idempotent recovery and promotion review | Same frozen plan and target population as `G5-OPS-002` | Same-plan resume recovered existing identities across the entire affected refinance closure, and the specialized lifecycle classes retain their independently reconciled unchanged replays. No duplicate or conflicting financial state was reported by apply or reconciliation. | Promotion decision accepted on 2026-09-13: same-plan durable recovery plus the unchanged canary matrix satisfies the idempotency gate. A portfolio-wide zero-write replay remains regression hardening, not a registry blocker. |
...
source signature rather than a loan-ID allowlist: state `3`, zero
`SALDO_TOTAL`, principal, both interest balances, mora, recargos, and CxC, with
only positive `SALDO_SEGURO` remaining. It freezes the residue in the lifecycle,
requires non-empty source-exact event reconstruction, and expects the native
closed loan to end with zero fee debt. Partial signatures retain ordinary
strict reconciliation and matching headers without reconstructable events are
quarantined. This is implemented and unit-proved, but the historical Gate 5
evidence predates it. Close the three closed-loan symptoms only after a fresh
clean cohort run and unchanged replay produce no fee-balance variance; do not
repair the current sandbox identities.

### `G5-SCH-008` classifier-derived acceptance check

The acceptance cohort is executable and comes only from the frozen plan; it is
not the historical 48-loan investigation set. A full plan may be used, but a
fresh namespaced plan that requests every current classifier member and expands
their complete dependency graph is sufficient. After the run and its strict
reconciliation have been recorded, run:

```bash
./arissto-sync check-loan-contractual-anchor-cohort --target local \
  --cycle CYCLE_ID --plan PLAN_ID --run RUN_ID --replay-run REPLAY_RUN_ID
```

The checker requires:

- at least one action classified as `source-exact-contractual-origin-anchor`;
- every non-quarantined classifier member to be `succeeded`, `recovered`, or
  `unchanged` in the accepted run;
- the classifier-derived cohort quarantine set to remain exactly the reviewed
  voided successor `638`; unrelated full-plan quarantines belong to their own
  Gate 5 rows and do not change this schedule-anchor acceptance set;
- a persisted successful reconciliation with no schedule mismatch for a
  classifier member; and
- an unchanged replay in which every supported classifier member is only
  `recovered` or `unchanged`, followed by another successful reconciliation
  with no classifier schedule mismatch.

The checker is read-only and uses persisted cycle evidence; it does not need a
running Fineract service. Accepted plan `817bb2ce525945b59e9358a89551e351`,
run `2ab37f99b9a546b0aa30e76b752b7c45`, and replay
`4e81b505c27443c0badfbc45d7bf69c4` derive 51 members, validate all 50 supported
members plus reviewed quarantine `638`, and return `accepted=true` with both
strict reconciliations recorded.

### `G5-SCH-014` contractual-anchor implementation progress

The planner now freezes an aggregate adjustment-history count from
`CRD_REESTRUCTURACION` and selects the active source-exact writer only when the
entire current schedule proves the contractual-origin rule. The classifier is
source-derived: it requires supported monthly or biweekly frequency, sequential
and strictly increasing installments, exact full principal, no current
restructure/deferred marker, no adjustment history, and exact Actual/Actual
interest from `FECHA_OTORGAMIENTO` for every period. Manual adjustments cannot
enter this path.

The writer preserves the real disbursement event and uses the contractual
origin as the first imported schedule period's `fromDate`. The Fineract identity
guard accepts the normal `ARISSTO:CRD:{id}` identity and a bounded lowercase
`PROOF:{namespace}:ARISSTO:CRD:{id}` canary identity; it still rejects unrelated
or nonnumeric external IDs.

Fresh local proof namespace `g5sch014-0903-v2` produced plan
`e7965df54ba847d4806e26317dfaff40`. It classified loan `280` with contractual
start `2023-10-31`, distinct from native nominal start `2023-10-30`, selected
`fineract-source-exact-active-schedule-v1`, and expanded the dependency to
predecessor `135`. After prerequisite recovery, retry run
`52a1ce1571fd489498d719d4d3d13ba5` recovered both loans. Strict reconciliation
returned `ok=true`, two loans, one product, two staff assignments, zero
mismatches, and no failed or quarantined entity. Its interest, penalty, and
cutover-total differences are explicitly accepted native-allocation variances;
they are not schedule mismatches. This proves the new path on one month-end
refinance-root canary but does not complete the 36-loan cohort or unchanged
replay.
...
marker. The actual financial disbursement is preserved independently and may
precede `FECHA_OTORGAMIENTO`; the contractual accrual anchor remains
`FECHA_OTORGAMIENTO - 1 day`. This includes the 14 reviewed roots `1`, `2`, and
`28` through `39`, and releases their 22 otherwise-supported refinance
descendants from propagated quarantine. A partial signature emits
...
rows and unsupported refinance graph shapes are quarantined. This was the
evidence boundary when the registry was changed to `blocked`; the accepted
2026-09-13 cycle has since promoted it to `available`.

The controlled local matrix now proves, for every regular selected loan:
...
- the existing reversal, terminal-state, refinance, accounting, and journal
  requirements; and
- idempotent recovery or an unchanged canary replay with no duplicate financial
  entry.
...
The warning above applies to the older default tenant only. The later clean
cycle `sandbox-2026-09-05-clean-a` exercised the supported portfolio on fresh
identities, including normal and Cobro Movil repayments, reversals, refinance
chains, and the source-exact schedule paths. Its full replay recovered the
supported population after the loan-`2374` fix, and scoped run
`7968a558d28747949bded7500bb1eaee` repaired and reconciled the final
`1507 -> 1663` failure. Fresh/clean cycle `sandbox-2026-09-13-loans-b`
subsequently closed the portfolio-level Gate 5 proof described in the current
status section; no further schedule-writer implementation pass remains.
...
canary-proved. The later clean cycle also exercised the formerly distinct
schedule/API edge population successfully, and the accepted 2026-09-13 cycle
closed the remaining portfolio-level promotion proof. Complete Mobile
Collections repayment linking remains downstream of that accepted loan run.

This completes implementation and controlled local full-target acceptance.
Mobile Collections may now consume the accepted native loan and repayment
identities after its own dependencies and readiness checks pass. Follow the
canonical [loans-to-mobile flow](../orchestration.md#current-loans-to-mobile-collections-flow).
