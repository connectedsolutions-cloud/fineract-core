...
public class SearchReadPlatformServiceImpl implements SearchReadPlatformService {
    ...
    public List<SearchData> retriveMatchingData(final SearchConditions searchConditions) {
        ...
        this.dataScopeService.bindNamedParams(params);
    ...
    public String searchSchema(final SearchConditions searchConditions) {
        ...
        final String clientMatchSql = "( (select 'CLIENT' as entityType, c.id as entityId, c.display_name as entityName, c.external_id as entityExternalId, c.account_no as entityAccountNo "
                ...
                + " from m_client c join m_office o on o.id = c.office_id where 1=1 " + this.dataScopeService.namedSqlForClient("c")
                + " and (c.account_no like :search or c.display_name like :search or c.external_id like :search or c.mobile_no like :search)) "
                + " order by c.id desc)";
        ...
        final String loanMatchSql = "( (select 'LOAN' as entityType, l.id as entityId, pl.name as entityName, l.external_id as entityExternalId, l.account_no as entityAccountNo "
                ...
                + " from m_loan l left join m_client c on l.client_id = c.id left join m_group g ON l.group_id = g.id left join m_product_loan pl on pl.id=l.product_id where 1=1 "
                + this.dataScopeService.namedSqlForLoan("l", "c") + " and (l.account_no like :search or l.external_id like :search)) "
                + " order by l.id desc)";
        ...
        final String savingMatchSql = "( (select 'SAVING' as entityType, s.id as entityId, sp.name as entityName, s.external_id as entityExternalId, s.account_no as entityAccountNo "
                ...
                + " from m_savings_account s left join m_client c on s.client_id = c.id left join m_group g ON s.group_id = g.id left join m_savings_product sp on sp.id=s.product_id "
                + " where 1=1 " + this.dataScopeService.namedSqlForSavings("s", "c")
                + " and (s.account_no like :search or s.external_id like :search)) " + " order by s.id desc)";
        ...
        final String shareMatchSql = "( (select 'SHARE' as entityType, s.id as entityId, sp.name as entityName, s.external_id as entityExternalId, s.account_no as entityAccountNo "
                ...
                + " from m_share_account s left join m_client c on s.client_id = c.id left join m_share_product sp on sp.id=s.product_id "
                + " where 1=1 " + this.dataScopeService.namedSqlForShare("c")
                + " and (s.account_no like :search or s.external_id like :search)) " + " order by s.id desc)";
        ...
        final String clientIdentifierMatchSql = "( (select 'CLIENTIDENTIFIER' as entityType, ci.id as entityId, ci.document_key as entityName, "
                ...
                + " from m_client_identifier ci join m_client c on ci.client_id=c.id " + " where 1=1 "
                + this.dataScopeService.namedSqlForClient("c") + " and ci.document_key like :search ) " + " order by ci.id desc)";
        ...
        final String groupMatchSql = "( (select CASE WHEN g.level_id=1 THEN 'CENTER' ELSE 'GROUP' END as entityType, g.id as entityId, g.display_name as entityName, g.external_id as entityExternalId, g.account_no as entityAccountNo, "
                ...
                + " from m_group g join m_office o on o.id = g.office_id where 1=1 "
                + this.dataScopeService.namedSqlForOfficeOnly("g.office_id")
                + " and (g.account_no like :search or g.display_name like :search or g.external_id like :search )) "
                + " order by g.id desc)";
