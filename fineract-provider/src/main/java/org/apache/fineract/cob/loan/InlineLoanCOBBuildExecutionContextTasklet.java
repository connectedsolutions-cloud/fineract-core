...
public class InlineLoanCOBBuildExecutionContextTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        ...
        // Check if any loan is in simulation mode - if so, use the businessDate (current processing day) as simulated
        // date
        ...
        for (Long loanId : loanIds) {
            ...
            if (loan != null) {
                log.info(
                        "Loan [{}]: isSimulation={}, simulatedDate={}, lastClosedBusinessDate={}, accruedTill={}, periodicAccrualEnabled={}",
                        loanId, loan.getIsSimulation(), loan.getSimulatedDate(), loan.getLastClosedBusinessDate(), loan.getAccruedTill(),
                ...
                if (Boolean.TRUE.equals(loan.getIsSimulation()) && loan.getSimulatedDate() != null) {
                    ...
                    // This ensures DateUtils.getBusinessLocalDate() returns the current day being processed, not the
                    // final simulated date
                    ThreadLocalContextUtil.setLoanSimulatedDate(businessDate);
                    log.info(
                            "Simulation loan detected: Setting ThreadLocal simulated date to CURRENT processing day [{}] (loan's final simulatedDate is [{}])",
        ...
        // Set business dates - use businessDate (current processing day) for simulation, or regular business date for
        // normal COB
