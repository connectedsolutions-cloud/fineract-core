...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  ...
  CREATE DATABASE ${FINERACT_TENANT_SANDBOX_DB_NAME:-fineract_sandbox};
  ...
  GRANT ALL PRIVILEGES ON DATABASE ${FINERACT_TENANT_SANDBOX_DB_NAME:-fineract_sandbox} TO $FINERACT_DB_USER;
  \c $FINERACT_TENANTS_DB_NAME
  GRANT ALL ON SCHEMA public TO $FINERACT_DB_USER;
  \c $FINERACT_TENANT_DEFAULT_DB_NAME
  GRANT ALL ON SCHEMA public TO $FINERACT_DB_USER;
  \c ${FINERACT_TENANT_SANDBOX_DB_NAME:-fineract_sandbox}
  GRANT ALL ON SCHEMA public TO $FINERACT_DB_USER;
EOSQL