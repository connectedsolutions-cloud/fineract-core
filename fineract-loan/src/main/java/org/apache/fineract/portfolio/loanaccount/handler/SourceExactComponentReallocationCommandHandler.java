...
@CommandType(entity = "LOAN", action = "SOURCEEXACTCOMPONENTREALLOCATION")
public class SourceExactComponentReallocationCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        try {
            return operationalMigrationService
                    .execute(() -> this.writePlatformService.makeSourceExactComponentReallocation(command.getLoanId(), command));
        } catch (final JpaSystemException | DataIntegrityViolationException dve) {
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, dve.getMostSpecificCause(), dve,
                    "loan.source.exact.component.reallocation", "Source-exact component reallocation");
