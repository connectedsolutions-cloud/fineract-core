...
@CommandType(entity = "LOAN", action = "SOURCEEXACTACCRUALCATCHUP")
public class SourceExactAccrualCatchupCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        try {
            return operationalMigrationService
                    .execute(() -> this.writePlatformService.materializeSourceExactAccruals(command.getLoanId(), command));
            ...
            dataIntegrityErrorHandler.handleDataIntegrityIssues(command, exception.getMostSpecificCause(), exception,
                    "loan.source.exact.accrual.catchup", "Source-exact accrual catch-up");
