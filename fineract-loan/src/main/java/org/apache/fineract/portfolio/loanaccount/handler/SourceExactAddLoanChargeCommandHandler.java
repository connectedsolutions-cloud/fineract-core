...
@CommandType(entity = "LOANCHARGE", action = "SOURCEEXACTCREATE")
public class SourceExactAddLoanChargeCommandHandler implements NewCommandSourceHandler {
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        return operationalMigrationService.execute(() -> {
            try {
                return writePlatformService.addLoanCharge(command.getLoanId(), command);
                ...
                dataIntegrityErrorHandler.handleDataIntegrityIssues(command, exception.getMostSpecificCause(), exception,
                        "loan.source.exact.charge", "Source-exact loan charge");
