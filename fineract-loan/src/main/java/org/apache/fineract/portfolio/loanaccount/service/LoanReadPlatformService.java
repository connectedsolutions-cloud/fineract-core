...
public interface LoanReadPlatformService {
     ...
     */
    Page<LoanAccountData> retrievePendingDisbursementsByExpectedDisbursedOnDateRange(LocalDate fromDate, LocalDate toDate,
            Long currentOfficeId, Integer limit, Integer offset);

    /**
     * Retrieves repayment schedule installments for active loan accounts whose due date falls within the inclusive
     * range [fromDate, toDate], filtered by the current office hierarchy if needed.
     *
     * The returned items are aggregated per loan account, and include the scheduled amount and completion status.
     */
    Page<RepaymentScheduledItemData> retrieveRepaymentScheduledByDueDateRange(LocalDate fromDate, LocalDate toDate, Long currentOfficeId,
